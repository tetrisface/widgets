...
local antiRangesDrawTimer = Spring.GetTimer()
local antiRangesDrawIntervalMs = 33
...
function widget:DrawWorldPreUnit()
	if chobbyInterface or Spring.IsGUIHidden() or Spring.DiffTimers(Spring.GetTimer(), antiRangesDrawTimer, true) < antiRangesDrawIntervalMs then
    return
  end
  antiRangesDrawTimer = Spring.GetTimer()
	local camX, camY, camZ = spGetCameraPosition()
  local nTotalAntis = 0

    for uID, pos in pairs(antiInLos) do
      nTotalAntis = nTotalAntis + 1
    ...
    antiRangesDrawIntervalMs = math.max(100, 100+ nTotalAntis / 10)
    Spring.Echo("Anti Ranges", 'nTotalAntis', nTotalAntis, 'antiRangesDrawIntervalMs' , antiRangesDrawIntervalMs)
end
