function widget:GetInfo()
	return {
		name = "Sensor Ranges LOS 2",
		desc = "Shows LOS ranges of all units. (GL4)",
		author = "Beherith GL4, Borg_King",
		date = "2021.06.18",
		license = "Lua: GPLv2, GLSL: (c) Beherith (mysterme@gmail.com), Edited by Juzza, SkyTurtle (2nd VBO), Rippsy (Merged PR #2554 updates)",
		layer = 0,
		enabled = true
	}
end
...
local function initgl4()
	...
	if circleInstanceVBO2 then
		clearInstanceTable(circleInstanceVBO2)
	end
	circleShader = LuaShader.CheckShaderUpdates(shaderSourceCache,0)

	if not circleShader then
		goodbye("Failed to compile losrange shader GL4 1")
	end
	...
	circleInstanceVBO2 = makeInstanceVBOTable(circleInstanceVBOLayout, 128, "losrangeVBO", 2)
	circleInstanceVBO2.numVertices = numVertices
	circleInstanceVBO2.vertexVBO = circleVBO
	circleInstanceVBO2.VAO = makeVAOandAttach(circleInstanceVBO2.vertexVBO, circleInstanceVBO2.instanceVBO)
end
...
local function InitializeUnits()
	...
	uploadAllElements(circleInstanceVBO2)
end
...
function widget:VisibleUnitAdded(unitID, unitDefID, unitTeam, noupload)
	...
	instanceCache[1] =  unitRange[unitDefID] 
	pushElementInstance(circleInstanceVBO2,
		instanceCache,
		unitID, --key
		true, -- updateExisting
		noupload,
...
function widget:VisibleUnitRemoved(unitID)
	if circleInstanceVBO.instanceIDtoIndex[unitID] then
		...
		if circleInstanceVBO2.instanceIDtoIndex[unitID] then
		popElementInstance(circleInstanceVBO2, unitID)
	end
end

function widget:DrawWorld()
	-- if spec and fullview then return end
	-- Rippsy, comment out line below to keep LOS visible during hidden ui
	...
	if circleInstanceVBO2.usedElements > 0 then
		-- Clear stencil mask
		gl.Clear(GL.STENCIL_BUFFER_BIT)
		-- Draw enemy LOS circles
		glColorMask(false, false, false, false) -- disable color drawing
		glStencilFunc(GL_NOTEQUAL, 1, 1) -- Always Passes, 0 Bit Plane, 0 As Mask
		glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE) -- Set The Stencil Buffer To 1 Where Draw Any Polygon
		glStencilMask(1) -- Only check the first bit of the stencil buffer
		circleInstanceVBO2.VAO:DrawArrays(GL_TRIANGLE_FAN, circleInstanceVBO2.numVertices, 0, circleInstanceVBO2.usedElements, 0)
		glColorMask(true, true, true, true)	-- re-enable color drawing
		glStencilFunc(GL_NOTEQUAL, 1, 1)
		glLineWidth(rangeLineWidth * lineScale * 1.0)
		circleInstanceVBO2.VAO:DrawArrays(GL_LINE_LOOP, circleInstanceVBO2.numVertices, 0, circleInstanceVBO2.usedElements, 0)
	end
