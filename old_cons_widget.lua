...
function getTraveltime(unitDef, A, B)
    ...
    if isTimeToMoveOn(secondsLeft, builderId, unitDef, totalBuildSpeed) and not buildingWillStall(secondsLeft, unitDef, totalBuildSpeed, secondsLeft) then
...
function buildingWillStall(buildingUnitId, buildingDef, totalBuildSpeed, secondsLeft)
    if not secondsLeft then
        secondsLeft = getBuildTimeLeft(buildingUnitId)
    end
    if not buildingDef then
        buildingDef = unitDef(buildingUnitId)
    end
    if not totalBuildSpeed then
        totalBuildSpeed = getBuildersBuildSpeed(getUnitsBuildingUnit(buildingUnitId))
    end
