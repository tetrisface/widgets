...
local overkillRatio             = 1.8


function widget:GameFrame(n)
  -- check maneuver units only
  ...
  -- for i = 1, #units do
  --   local unit = units[i]
  --   if not avoidTargets[unit.id] then
  --     table.insert(retargetUnits, unit)
  --   end
  -- end
    ...
    if unit.team == myTeamId then
      ...
      if unitStates and unitStates.firestate == 2 then
        ...
        for j = 1, #inRangeUnits do
          if not table.has_value(allies, GetUnitTeam(inRangeUnits[j])) then
            table.insert(inRangeEnemyUnits, inRangeUnits[j])
          end
        end
        if inRangeEnemyUnits and #inRangeEnemyUnits > 0 then
          table.sort(inRangeEnemyUnits, function(a, b)
            ...
            return table.has_value(avoidTargets, a['id']) < table.has_value(avoidTargets, b['id']) and (aIncTot > overkillRatio) > (bIncTot > overkillRatio) and aIncTot > bIncTot
          end)
          ...
          -- log(unit.id .. ' attack ' .. inRangeEnemyUnits[1])
          SetUnitTargetasdf(unit.id, inRangeEnemyUnits[1])
        end
      end
    end
    -- end
  ...
  -- table.sort(targets, function(a, b) return a['incomingDamage'] / a['totalHealth'] > b['incomingDamage'] / b['totalHealth'] end)
end
...
function registerUnit(unitID, unitDefID, unitTeam)
  ...
  for j = 1, #unitDef.weapons do
    ...
    maxRange = math.max(maxRange, unitDef.weapons[j].range or 0, unitDef.wDefs[j].range or 0)
