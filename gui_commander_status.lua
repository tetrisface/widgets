...
local circleInstanceVBOLayout = {
	{ id = 0, name = 'screenPosScale', size = 4 }, -- position (x,y) + scale (a,b)
	{ id = 1, name = 'healthRatio', size = 1 },
}
...
layout(location = 0) in vec4 screenPosScale;      // unit circle vertex positions
layout(location = 1) in float healthRatio;      // center (x,z) and radius (w); posscale.y unused here

uniform vec4 screenPosScale;
...
function widget:Initialize()
	...
	for unitDefId, unitDef in pairs(UnitDefs) do
		if unitDef.isBuilding and (unitDef.hasShield or (unitDef.customparams and unitDef.customparams.shield_radius and unitDef.customparams.shield_radius > 0)) then
...
function widget:DrawWorld()
	...
	gl.UniformFloat(pulseAlphaUniform, pulseMs < 500 and (0.1 + (0.35 - 0.1) * (pulseMs / 499)) or (0.35 + (0.1 - 0.35) * ((pulseMs - 500) / 499)))
