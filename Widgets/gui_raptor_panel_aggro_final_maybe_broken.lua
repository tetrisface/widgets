...
function widget:GetInfo()
  return {
    name = 'Raptor Stats Panel With Eco Attraction (Broken)',
...
local function DrawPlayerAttractions(stage)
  ...
  for i = 1, #playerEcoAttractionsRender do
    ...
    font:Print(
      CutStringAtPixelWidth(playerEcoAttraction.name, valuesLeftX - namePosX - 2),
      namePosX,
      rowY,
      panelFontSize
    )
...
local function CreatePanelDisplayList()
  ...
  elseif stage == stageBoss then
    if isRaptors then
      printPanel(
        I18N('ui.raptors.queenHealth', {count = nBosses, health = ''}):gsub('%%', ''),
        panelMarginX,
        PanelRow(1)
      )
    ...
    if bossInfo then
      printBossInfo(
        (isRaptors and 'Queen' or 'Boss') .. ' resistances: (Ctrl+B Expands)',
        bossInfoMarginX,
        PanelRow(11)
      )
      local row = 11
